...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        breedRand = Object.keys(breedsObject)
...
select.addEventListener('change', event => {
    ...
    breedSelect = event.target.value
    getDoggoImg(url)
    generate()
...
document.querySelector('.refresh').addEventListener('click', () => {
    if (breedSelect !== undefined){
            let url = `https://dog.ceo/api/breed/${breedSelect}/images/random`
    getDoggoImg(url)
    generate()
    }
...
document.querySelector('.random').addEventListener('click', () => {
    let choice = breedRand[Math.floor(Math.random() * breedRand.length)]
    const option = document.createElement('option');
            option.value = choice;
...
const doggoInfo = {
    ...
    assignAge() {
        this.age = Math.floor(Math.random() * 12 + 1)
